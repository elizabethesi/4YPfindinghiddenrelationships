...
"""This the granger function: It plots and saves the Hinton diagrams for the a_hat matrix
(the size of the coefficients for each lagged variable), the Hinton diagram for the T-test
which asesses how probable it is that the coefficients are actually 0, and the graph which 
depicts the original data over time against that of the 2 models"""

def granger_function(adj_prices, density_plane_log, order_num , stock, pl_key):
	import numpy as np
	...
	for a in density_plane_log:
		...
		den_plane_log_stan.append(b)

	adj_pri_stan = []
	for a in adj_prices:
		b = float(a-mean_pri)/float(sd_pri)
		adj_pri_stan.append(b)	
	...
	#following steps from http://reliawiki.org/index.php/Multiple_Linear_Regression_Analysis
	...
	for a in range(0,2*order):
		#want the diagonal components of the covariance matrix
		val = abs(C_adj[a,a])
		if val >0:
	...
	plt.savefig("Grangerfinal/"+str(stock)+"/T_test"+str(order_num)+"Com"+str(stock1)+"_Pl"+str(pl_key)+".png")
	plt.close()
	...
	plt.plot(x_a, adj_pri_stan[order:], color ='b', linewidth=0.5)
	plt.plot(x_a,model_adj, color ='g', linewidth=0.5)
	plt.plot(x_a,Resmodel_adj, color ='r', linewidth=0.5)
	...
	plt.savefig("Grangerfinal/"+str(stock)+"/Order"+str(order_num)+"Com"+str(stock)+"_Pl"+str(pl_key)+".png")
	plt.close()
	...
	hinton(a_hat_t)
	plt.savefig("Grangerfinal/"+str(stock)+"/AHAT"+str(order_num)+"Com"+str(stock)+"_Pl"+str(pl_key)+".png")
	plt.close()

	return p_value_adj, p_value_pl

	##T-test

	#for every beta value (on the off diagonal) calculate the absolute r (pearsons correlation coefficient)
